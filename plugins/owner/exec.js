...
const pluginConfig = {
    ...
    alias: ['>', 'run', 'execute'],
    category: 'owner',
    description: "Ejecutar el código JS del mensaje enviado (sólo usuario)",
    usage: ".> (respuesta mensajes que contienen código)",
    example: '.> (reply)',
    isOwner: true,
    isPremium: false,
    isGroup: false,
...
async function handler(m, { sock, store }) {
    ...
    if (!code) {
        return m.reply(
            `⚙️ *ᴇxᴇᴄ*\n\n` +
            `¡Responda a un mensaje que contiene código JavaScript!
...
` +
            ...
            `*Ejemplo:*
` +
            `> Responder al mensaje: \`return m.chat\`
` +
            `> Luego escriba: .>`
        )
    }
