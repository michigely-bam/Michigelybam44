...
const pluginConfig = {
  ...
  usage: '.fakedev2 <nombre> (responde o envía una foto)',
...
` +
        `*MODO DE USO:*\n` +
        `> 1. Envía una foto con el texto \`${m.prefix}fakedev2 <nombre>\`\n` +
        `> 2. Responde a la foto con \`${m.prefix}fakedev2 <nombre>\``,
    );
  }
  let buffer = null;
  if (
