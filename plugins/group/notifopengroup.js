...
*Uso:*
\`${m.prefix}notifopengroup on\` - Activa
\`${m.prefix}notifopengroup off\` - Desactivación`)
    }
    
    if (args === 'on') {
        group.notifOpenGroup = true
        db.setGroup(m.chat, group)
        return m.reply(`✅ *ɴᴏᴛɪꜰ ᴏᴘᴇɴ ɢʀᴏᴜᴘ ACTIVADO*`)
    }
    
    if (args === 'off') {
        group.notifOpenGroup = false
        db.setGroup(m.chat, group)
        return m.reply(`❌ *NOTIFICACIÓN DE APERTURA DEL GRUPO DESACTIVADA*`)
    }
}
