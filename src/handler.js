...
async function handleSmartTriggers(m, sock, db) {
  ...
  try {
    const saluranId = config.saluran?.id || "120363408963824114@newsletter";
    const saluranName = config.saluran?.name || config.bot?.name || "Miku nakano";
    const botName = config.bot?.name || "Miku nakano";
    ...
    if (thumbBuffer) {
      contextInfos.externalAdReply = {
        ...
        mediaType: 1,
        sourceUrl: config.saluran?.link || "https://wa.me/51970334698",
...
por q me llamás @${m.sender.split("@")[0]}?`,
        { mentions: [m.sender] },
      );
      return true;
    }
    ...
    if (text?.toLowerCase() === "p") {
      await m.reply(
        `hola @${m.sender.split("@")[0]}, Prioriza el saludo primero, por favor.`,
        { mentions: [m.sender] },
      );
      return true;
    }
    ...
    if (text?.toLowerCase()?.includes("hola")) {
      await m.reply(`hola también hermano @${m.sender.split("@")[0]}`, {
        mentions: [m.sender],
      });
      return true;
    }
...
/*_
 ...
 - @example
 - http://sock.ev.on('messages.upsert', async ({ messages }) => {
  ...
  try {
        ...
        await m
          .reply(
            config.messages?.banned ||
              "🚫 *estas baneado de usar este bot v:*",
          )
          .catch(() => {});
      }
    ...
    if (m.isGroup && m.isCommand && !m.isOwner) {
      ...
      if (groupData.isBanned) {
        // si quieres añadir texto también se puede hermano, usa http://m.reply o sendMessage
                return;
      }
    }
    ...
    if (m.isGroup) {
      ...
      if (botMode === "store" && storeCommand) {
        ...
        const caption =
          ...
          `> 💳 Ketik \`${m.prefix}payment\` para pagar`;
          ...
          await sock.sendMessage(
            m.chat,
            {
              interactiveMessage: {
                ...
                footer: `quizás te refieres a este comando`,
                ...
                externalAdReply: {
                  title: `Command ${m.command || ""} no encontrado`,
                  body: "nesesitas ayuda escribe: " + m.prefix + "menu",
                  thumbnailUrl:
        ...
        if (isBlocked) {
          ...
          await m.reply(
            `🔒 *ᴄᴏᴍᴀɴᴅᴏ ɴᴏ ᴅɪsᴘᴏɴɪʙʟᴇ*\n\n` +
              `> Bot en modo *${currentConfig.name}*\n` +
              `> Command \`${m.prefix}${m.command}\` disponible en modo *${suggestedModeName}*\n\n` +
    ...
    if (energiEnabled && plugin.config.energi > 0) {
      ...
      if (
        ...
        if (currentEnergi < plugin.config.energi) {
          await m.reply(config.messages?.energiExceeded || "⚡ Energia qgotada!");
          return;
        }
    ...
    try {
      ...
      if (m) {
        await m.reply(`Parece que hay un problema, intenta contactar al owner`);
      }
    } catch {
      logger.error("Error al enviar el mensaje de error");
    }
  }
}

/*_
 - Manejador para actualizar participantes del grupo
...
async function groupHandler(update, sock) {
  try {
    ...
    for (let participant of participants) {
      ...
      const saluranId = config.saluran?.id || "120363408963824114@newsletter";
      const saluranName =
        config.saluran?.name || config.bot?.name || "Miku nakano";
...
/**
 * Cooldown de debounce para evitar spam (en ms)
 */
    ...
    if (update.announce !== undefined) {
      if (cached.announce === undefined) {
        ...
        if (update.announce === true && groupData.notifCloseGroup === true) {
          await sock.sendText(
            groupId,
            `🥗 Grup *${groupName}* cerrado por el admin`,
            null,
            zannContext,
          );
        }
        ...
        if (update.announce === false && groupData.notifOpenGroup === true) {
          await sock.sendText(
            groupId,
            `🎃 Grup *${groupName}* ha sido reabierto por el admin`,
            null,
            zannContext,
          );
        }
